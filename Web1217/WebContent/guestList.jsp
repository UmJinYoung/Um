...
<body>
 ...
 <table width=900 border=1 cellspacing=0>  
  ...
  <tr>
     ...
     <td> ${dto.name} </td>
     <td> 
      <a href="detail.do?idx=${dto.sabun}"> ${dto.title}</a><font style="font-size:12pt; color:red;"><c:if test="${dto.reply>0}">[${dto.reply}]</c:if> </font>                             
     </td>
     <td> ${dto.getPay() } </td>
  </tr>   
  </c:forEach>
  	...
  	<td colspan="5">
  	 <form name="myform" action="list.do">
  	 	...
  	 	<select name="keyfield" onchange="changeclear();">
  	 	   ...
  	 	   <option value="name" <c:if test="${! empty AA}">selected</c:if> >이름조회</option>
  	 	   <option value="title" <c:if test="${BB != ''}">selected</c:if> >제목검색</option>
